import {APlugin ,AMessage,findIndexByName,Strand,getNonZeroKeys,startstatus,stopstatus,gettupo,getstring,checkZeroValue,checkAllZeroValues,
    ...
    findThings,
    AEvent,
	createPlayerObject} from '../../api'
import {existplayer,Read_player,Write_player,武者境界, 灵魂境界,体魄境界,finduid,装备列表,transaction_record,findTransaction, updateTransaction,auction_record, all_zongmen, 武圣塔} from '../../model/gameapi';
let shezhi = []
export class tower extends APlugin  {
	...
	async my_tower(e:AEvent){
		...
		let tower:any = await 武圣塔.findOne({where:{uid:player.uid},raw:true});
		if(!tower) return e.reply(`你没有武圣塔`);
		let msg2:any[] = [];
		if(tower.挑战者.lenght > 0){
			let maxChallenge = [tower.挑战者.reduce((max, current) => {
    ...
    async check_tower(e:AEvent){
        let [page] = e.msg.replace(/(\/|#)?查看武圣塔/, "").trim().split("*").map(code => code.trim());
	...
	async challenge_tower(e:AEvent){
		...
		if (!await existplayer(1, usr_qq)) return false;
		let { player } = await Read_player(1, usr_qq);
		const [tower_name] = e.msg.replace(/(\/|#)?挑战武圣塔/, "").trim().split("*").map(code => code.trim());
		const tower:any = await 武圣塔.findOne({ raw: true, where: { name: tower_name } });
	...
	async create_tower(e:AEvent){
		...
		if(await 武圣塔.findOne({where:{uid:player.uid}})) return e.reply(`你已经创建过武圣塔`);
	...
	async create_tower2(e:AEvent){
		...
		await 武圣塔.create({name:e.msg,武圣名:player.name,武圣实力:A_player,uid:player.uid,挑战者:[]});
		this.cancel();
		e.reply(`武圣塔创立成功`)
	}
}