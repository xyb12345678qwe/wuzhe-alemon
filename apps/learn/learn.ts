...
export class learn extends plugin {
    constructor() {
        super({
            /** 功能名称 */
            name: 'learn',
        ...
        async learn(e:AMessage):Promise<boolean>{
            ...
            let [player, bag, gongfa_list, gongfa] = await Promise.all([
                Read_player(1,true, usr_qq, "player"),
                Read_player(1,true, usr_qq, "bag"),
                Read_json(3, "/item/功法列表.json"),
                Read_json(3, "/item/功法列表.json").then(daoju_list => daoju_list.find((item: { name: string; }) => item.name === thing_name)),
            ]);
        ...
        async visit(e:AMessage):Promise<boolean>{
            ...
            const [player, bag, gongfa_list, peizhi] = await Promise.all([
                Read_player(1, true, usr_qq, "player"),
                Read_player(1, true, usr_qq, "bag"),
                Read_json(3, "/item/功法列表.json"),
                Read_json(2, '')
